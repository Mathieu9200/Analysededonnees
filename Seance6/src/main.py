...
# Isoler la colonne « Surface (km²) » et la convertir en liste de float
surface = list(iles["Surface (km²)"].dropna().astype(float))
...
# Ordonner la liste avec la fonction locale
surface_ordonnee = ordreDecroissant(surface)

# Visualiser la loi rang-taille
plt.figure(figsize=(8,6))
plt.plot(range(1, len(surface_ordonnee)+1), surface_ordonnee, marker='o')
plt.xlabel("Rang")
...
plt.tight_layout()
plt.savefig("rang-taille-surfaces.png")
plt.show()
# ...existing code...
